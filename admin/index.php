...
$uRole = !empty($loggedInUser['is_admin']) ? 'Admin' : 'User';
$isAdmin = !empty($loggedInUser['is_admin']);

// Resolve role label from role_master using role_id
if (!$isAdmin && !empty($loggedInUser['role_id'])) {
    try {
        $pdo = getDbConnection();
        $roleStmt = $pdo->prepare("SELECT * FROM role_master WHERE role_id = ? LIMIT 1");
        $roleStmt->execute([(int) $loggedInUser['role_id']]);
        $roleRow = $roleStmt->fetch();
        if ($roleRow) {
            foreach (['role_name', 'role', 'name', 'title'] as $key) {
                if (isset($roleRow[$key]) && trim((string) $roleRow[$key]) !== '') {
                    $uRole = (string) $roleRow[$key];
                    break;
                }
            }
        }
    } catch (Throwable $e) {
        // Keep fallback role label
    }
}
?>
<!DOCTYPE html>
